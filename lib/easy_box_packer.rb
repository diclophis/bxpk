...
module EasyBoxPacker
  class << self
    def pack(container, items)
      ...
      items.each do |item|
        ...
        packings.each do |packing|
          ...
          packing[:spaces] = packing[:spaces].reject { |space| space[:dimensions].inject(:*) == 0 }
          ...
          packing[:spaces].each do |space|
            ...
            packing[:spaces] = packing[:spaces].reject { |x| x == space }
    ...
    def find_smallest_container(in_items)
      items = in_items.map { |i| i.delete(:weight); i }
      ...
      find_possible_container(
        possible_containers,
        invalid_containers,
        based_container[:dimensions],
        sorted_items.map {|i| i[:dimensions]},
        1,
        min_vol
      )
    ...
    def find_possible_container(possible_containers, invalid_containers, container, items, item_index, min_vol)
      ...
      [minimum_std].uniq.compact.each do |cont|
        find_possible_container(possible_containers, invalid_containers, cont, items, (item_index + 1), min_vol)
      end
    end
    ...
    def item_greedy_box(items)
      ...
      items_total_height      = array_of_lwh.inject(0) { |sum, x| sum+=x[2] }.round
